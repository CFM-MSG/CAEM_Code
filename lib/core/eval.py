...
def dr_eval(segments, data):
    ...
    for seg, dat in zip(segments, data):
        ...
        gt_start = dat['times'][0]
        gt_end = dat['times'][1]

        for i,t in enumerate(tious):
            for j,r in enumerate(recalls):
                p_start = np.array(seg)[:r, 0]
                p_end = np.array(seg)[:r, 1]
                tmp_1 = 1 - abs(p_start - gt_start) / duration
                tmp_2 = 1 - abs(p_end - gt_end) / duration
                tmp_dr = tmp_1 * tmp_2
                eval_result[i][j].append((overlap > t)[:r].any() * max(tmp_dr))
    eval_result = np.array(eval_result).mean(axis=-1)
    miou = np.mean(average_iou)

    return eval_result, miou

def eval_predictions(segments, data, metric, verbose=True):
    assert metric in ['R', 'dR']
    if metric == 'R':
        eval_result, miou = eval(segments, data)
    else:
        eval_result, miou = dr_eval(segments, data)
    if verbose:
