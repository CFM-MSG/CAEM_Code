...
class TLocVLBERT(nn.Module):
    def __init__(self, dataset, config):
        ...
        if dataset ==  "ActivityNet":
            ...
            # iou_mask_map = torch.zeros(37,37).float()
            # for i in range(0,36,1):
            #     iou_mask_map[i,i+1:min(i+17,37)] = 1.
            # for i in range(0,36-16,2):
            #     iou_mask_map[i,range(18+i,37,2)] = 1.
        ...
        self.ano_vlbert = VisualLinguisticBert(dataset, config, language_pretrained_model_path=language_pretrained_model_path)
        ...
        self.proj_txt_concept = torch.nn.Sequential(
                torch.nn.Linear(dim, config.CLASSIFIER_HIDDEN_SIZE),
                torch.nn.ReLU(inplace=True),
                torch.nn.Dropout(config.CLASSIFIER_DROPOUT, inplace=False),
                torch.nn.Linear(config.CLASSIFIER_HIDDEN_SIZE, config.PROJECTOR_LATENT_SIZE)
        ...
        self.proj_vid_concept = torch.nn.Sequential(
                torch.nn.Linear(dim*3, config.CLASSIFIER_HIDDEN_SIZE),
                torch.nn.ReLU(inplace=True),
                torch.nn.Dropout(config.CLASSIFIER_DROPOUT, inplace=False),
                torch.nn.Linear(config.CLASSIFIER_HIDDEN_SIZE, config.PROJECTOR_LATENT_SIZE)
    ...
    def forward(self, text_input_feats, text_mask, word_mask, object_visual_feats, num=0, flag=False):
        ...
        if num == 0:
            ...
            hidden_states_text, hidden_states_object, _, _ = self.ano_vlbert(text_input_feats, text_mask, word_mask, object_visual_feats, output_all_encoded_layers=False)
        ...
        vid_concept = self.proj_vid_concept(visual_point).reshape(bsz,T*T,-1)
        txt_concept = self.proj_txt_concept(text_point).unsqueeze(1)
        corss_similarity = torch.cosine_similarity(vid_concept, txt_concept, dim=-1).reshape(bsz,T,T)

        logits_visual = torch.cat((self.final_mlp_s(hidden_s), self.final_mlp_e(hidden_e), self.final_mlp_c(hidden_c)), -1)

        return logits_text, logits_visual, logits_iou, self.iou_mask_map.clone().detach(), visual_content_emb, text_query_emb, corss_similarity
